...
const CartPage = () => {
  ...
  const handleWhatsAppOrder = () => {
    ...
    const footer = `\n\nSubtotal: ₹${totalPrice}\n(Delivery charges of ₹20/km will be added based on location)`;
    const url = `https://wa.me/9390007617?text=${encodeURIComponent(
      "Order from Sri Vasavi Family Dhaba:\n\n" + message + footer
    )}`;
    window.open(url, "_blank");
  };
      ...
      <div className="container mx-auto px-4 py-12 max-w-4xl">
        {items.length === 0 ? (
          ...
          <>
            ...
            <div className="mt-8 p-6 bg-dhaba-warm/5 rounded-2xl border border-dhaba-warm/20 flex items-center gap-4">
              <div className="p-3 bg-dhaba-yellow/20 rounded-full text-dhaba-dark">
                <MapPin size={24} />
              </div>
              <div>
                <h4 className="font-display font-bold text-lg text-dhaba-dark">Delivery Information</h4>
                <p className="text-sm text-muted-foreground italic">Note: Extra ₹20 per km will be added to the final bill based on delivery distance.</p>
              </div>
            </div>

            {/* Order Summary */}
            <div className="mt-6 bg-dhaba-dark text-dhaba-cream p-8 rounded-3xl shadow-2xl relative overflow-hidden">
              <div className="absolute top-0 right-0 w-32 h-32 bg-dhaba-yellow/10 rounded-full -mr-16 -mt-16" />
              
              <div className="flex items-center justify-between mb-8">
                <div>
                  <span className="font-body text-dhaba-cream/60 uppercase tracking-widest text-sm">Cart Total</span>
                  <h2 className="font-display text-4xl font-bold text-dhaba-yellow mt-1">₹{totalPrice}</h2>
                </div>
                <ShoppingCart size={40} className="text-dhaba-yellow/20" />
              </div>
              ...
              <p className="text-center text-xs text-dhaba-cream/40 mt-6 italic">
                * Final amount including delivery will be shared on WhatsApp.
              </p>
            </div>
          </>
        )}
